...
<div id="playlistContainer">
				...
				{#each Object.values(playlistsTemp) as preset}
					<tr>
						...
						<td
							...
							on:click={() => {
								changeselectedPlaylist(preset.id);
							}}
						>
							{preset.name}
						</td>
						...
						<td
							...
							style:height={'1rem'}
							on:click={() => (deleteModal = preset.id)}
						>
							<MdDelete />
						</td>
					</tr>
	...
	<div id="playlistMetaContainer">
		{#if selectedPlaylist}
			...
			<div>
				{#if $user.username == selPlay.owner}
			...
			<div style:grid-column="span 2">
				<center>Limits are total by user, -1 denotes no limit</center>
			</div>
			...
			<div>
				<input
					...
					min="-1"
					step="1"
					bind:value={selPlay.durationLimit}
					disabled={$user.username !== selPlay.owner}
				/>
			</div>
			<div class="metaHeader">Item Limit</div>
			<div>
				<input bind:value={selPlay.itemLimit} disabled={$user.username !== selPlay.owner} />
			</div>
			<div class="metaHeader">Allow Duplicates</div>
			<div>
				<input
					type="checkbox"
					bind:checked={selPlay.allowDuplicates}
					disabled={$user.username !== selPlay.owner}
				/>
			</div>
			...
			<div>
				<input
					...
					min="-1"
					step="1"
					bind:value={selPlay.deleteAfter}
					disabled={$user.username !== selPlay.owner}
				/>
			</div>
			<div class="metaHeader">Mode</div>
			<div>
				<select bind:value={selPlay.mode} disabled={$user.username !== selPlay.owner}>
			...
			<div>
				<select bind:value={selPlay.minAccessLevel} disabled={$user.username !== selPlay.owner}>
		...
		{#if selectedPlaylist}
			<div>
				<input
					...
					bind:value={override}
					disabled={$user.accessLevel < selPlay.minAccessLevel}
				/>
				<input
					placeholder="URL, seperate with comma for bulk"
					bind:value={url}
					disabled={$user.accessLevel < selPlay.minAccessLevel}
				/>
				<button
					...
					disabled={$user.accessLevel < selPlay.minAccessLevel}>Add to Playlist</button
				>
				<br />
			</div>
			<div class="scroller">
	...
	{#if deleteModal !== ''}
		...
		>
			...
			<button
				on:click={() => {
					deletePlaylist(playlistsTemp[deleteModal]);
					deleteModal = '';
				}}>Yes</button
			>
