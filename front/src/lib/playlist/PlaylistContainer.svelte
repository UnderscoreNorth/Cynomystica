<script lang="ts">
	...
	import ScheduleDebug from './ScheduleDebug.svelte';
	let mediaURL: string;
	let title: string;
	let permanent = false;
	...
	if ($user.accessLevel >= $permissions.viewDebug) tabs.push('Schedule Debug');
...
{#if selectedTab == 'Live Queue'}
	<div id="playlistControlContainer">
		<input bind:value={title} placeholder="Title Override" />
		<div>
		...
		<button
			on:click={queueNext}
			disabled={queueNextDisabled || $user.accessLevel < $permissions.queueNext}>Queue Next</button
		>
		<input bind:value={mediaURL} placeholder={'URL/iframe'} />
		<button on:click={queueLast} disabled={$user.accessLevel < $permissions.queueLast}
			>Queue Last</button
		>
	</div>
		...
		<table>
			...
			<tbody id="playlistBody">
				{#each $queue as item, i}
					...
					{:else}
						<tr class={item.permanent ? 'permanent' : 'temporary'}
							><PlaylistItem {item} {deleteItem} /></tr
						>
					{/if}
				{/each}
			</tbody>
...
<style>
	...
	#playlistControlContainer > *:nth-child(1) {
		grid-area: 1 / 1 / 2 / 2;
	}
	#playlistControlContainer > *:nth-child(2) {
		grid-area: 1 / 2 / 2 / 3;
	}
	#playlistControlContainer > *:nth-child(3) {
		grid-area: 1 / 3 / 2 / 4;
	}
	#playlistControlContainer > *:nth-child(4) {
		grid-area: 2 / 1 / 3 / 3;
	}
	#playlistControlContainer > *:nth-child(5) {
		grid-area: 2 / 3 / 3 / 4;
	}
	table th {
