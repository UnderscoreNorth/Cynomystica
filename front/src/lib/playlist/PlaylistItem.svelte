<script lang="ts">
	...
	export let item: any;
	export let deleteItem: Function | null = null;
	export let compact = false;
	let innerWidth = 0;
	const getURL = (type: string, url: string) => {
		if (type == 'iframe') return '';
		let provider = '';
		if (type == 'yt' || type == 'ytlive') provider = 'https://youtube.com/watch?v=';
		if (type == 'tw_live') provider = 'https://twitch.tv/';
		return provider + url;
	};
</script>
...
{#if compact}
	<td style={'line-height:normal;padding:0 0.5rem'}>
		{#if getURL(item.type, item.url)}
			<a href={getURL(item.type, item.url)} target="_blank" rel="noreferrer">{item.name}</a>
		{:else}
			{item.name}
		{/if}
		<br />
		{secondsToTime(item.duration)} - {item.username}
		<span style:float="right">{new Date(item.startDate).toLocaleTimeString()}</span>
	</td>
{:else if innerWidth < 768}
	<td>
		{#if ($user.accessLevel >= $permissions.manageQueue || $user.username == item.username) && deleteItem !== null}
